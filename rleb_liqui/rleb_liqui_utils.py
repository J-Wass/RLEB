...
def _get_page_id_from_url(liquipedia_url: str) -> str:
    ...
    liquipedia_url = liquipedia_url.replace("https://", '').replace("http://", '').replace("www.", '')
    ...
    if len(pages) != 1:
        raise Exception(f"Found {len(pages)} pages with name {liquipedia_page_title}! Excepted only 1.")

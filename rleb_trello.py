        ...
        while True:
            try:
                ...
                for action in actions:
                    ...
                    rleb_settings.rleb_log_info("TRELLO: Action - {0}".format(action["type"]))
                ...
                if rleb_settings.thread_crashes['thread'] > 5:
                    break
                rleb_settings.rleb_log_error("REDDIT: Monitoring Trello failed - {0}".format(e))
...
def get_trello_actions(date):
    ...
    action_request = "https://api.trello.com/1/boards/{0}/actions/?since={1}Z&key={2}&token={3}".format(rleb_settings.TRELLO_BOARD_ID, iso_date, rleb_settings.TRELLO_AUTH_KEY, rleb_settings.TRELLO_AUTH_TOKEN)
...
def human_readable_trello_action(action):
    ...
    if action["type"] == "commentCard":
        action["message"] = "Commented on '{0}'".format(action["data"]["card"]["name"])
    elif action["type"] == "createCard":
        action["message"] = "Created card '{0}'".format(action["data"]["card"]["name"])
    elif action["type"] == "updateCard":
        if "listAfter" in action["data"]:
            action["message"] = "Moved card '{0}' to list '{1}'".format(action["data"]["card"]["name"],action["data"]["listAfter"]["name"])
        elif "list" in action["data"]:
            action["message"] = "Moved card '{0}' to list '{1}'".format(action["data"]["card"]["name"],action["data"]["list"]["name"])
        else:
            action["message"] = "Updated card '{0}'".format(action["data"]["card"]["name"])
    elif action["type"] == "createList":
        action["message"] = "Created list '{0}'".format(action["data"]["list"]["name"])
    elif action["type"] == "updateList":
        action["message"] = "Updated list '{0}'".format(action["data"]["list"]["name"])
    else:

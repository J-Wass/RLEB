...
class TestGroupLookup(RLEBAsyncTestCase):
    ...
    async def test_group_lookup(self):
        ...
        def mock_liquipedia(args = []):
            with open('tests/resources/oce_major_groups.html', encoding="utf8") as f:
        ...
        with patch.object(rleb_stdout, 'print_to_channel') as mocked_print_to_channel:
            await rleb_group_lookup.handle_group_lookup('liquipedia url', mock_channel)
            ...
            mocked_print_to_channel.assert_awaited_once_with(mock_channel, expected_markup, title="Groups")

...
class TestCalendar(RLEBAsyncTestCase):
    ...
    async def test_reddit_events(self):
        ...
        with patch.object(stdout, "print_to_channel") as mocked_print_to_channel:
            await calendar_event.handle_calendar_lookup(mock_channel, "reddit", 7)
            ...
            mocked_print_to_channel.assert_awaited_once_with(
                mock_channel, expected_markup, title="reddit calendar for next 7 days", force_pastebin=True
            )
    ...
    async def test_google_sheets_events(self):
        ...
        with patch.object(stdout, "print_to_channel") as mocked_print_to_channel:
            await calendar_event.handle_calendar_lookup(mock_channel, "sheets", 7)
            ...
            mocked_print_to_channel.assert_awaited_once_with(
                mock_channel, expected_markup, title="sheets calendar for next 7 days", force_pastebin=True
            )

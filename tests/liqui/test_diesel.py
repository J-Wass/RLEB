...
class TestDiesel(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        def mock_diesel_request(url, *args, **kwargs):
            ...
            if "healthcheck" in url:
                ...
                return MockResponse(
                    "UHJpemVwb29sIG1hcmtkb3du"
                )  # "Prizepool markdown" in base64
            elif "swiss" in url:
                return MockResponse(
                    "U3dpc3MgbWFya2Rvd24="
                )  # "Swiss markdown" in base64
            elif "bracket" in url:
                return MockResponse(
                    "QnJhY2tldCBtYXJrZG93bg=="
                )  # "Bracket markdown" in base64
            elif "groups" in url:
                return MockResponse(
                    "R3JvdXAgbWFya2Rvd24="
                )  # "Group markdown" in base64
            elif "makethread" in url:
                return MockResponse(
                    "TWFrZXRocmVhZCBtYXJrZG93bg=="
                )  # "Makethread markdown" in base64
            elif "broadcast" in url:
                return MockResponse(
                    "QnJvYWRjYXN0IG1hcmtkb3du"
                )  # "Broadcast markdown" in base64
            elif "streams" in url:
                return MockResponse(
                    "U3RyZWFtcyBtYXJrZG93bg=="
                )  # "Streams markdown" in base64
            elif "schedule" in url:
                return MockResponse(
                    "U2NoZWR1bGUgbWFya2Rvd24="
                )  # "Schedule markdown" in base64
            elif "coverage" in url:
                return MockResponse(
                    "Q292ZXJhZ2UgbWFya2Rvd24="
                )  # "Coverage markdown" in base64
            elif "mvp_candidates" in url:
                return MockResponse(
                    "TVZQIGNhbmRpZGF0ZXM="
                )  # "MVP candidates" in base64
            return MockResponse("")

        self.mock_diesel_requests = patch.object(
            requests, "get", side_effect=mock_diesel_request
        ).start()
    ...
    async def test_get_bracket_markdown_date(self):
        ...
        result = await diesel.get_bracket_markdown_date(
            "https://liquipedia.net/test", 20220101
        )
        self.assertEqual(result, "Bracket markdown")
    ...
    async def test_handle_makethread_lookup(self):
        ...
        with patch(
            ...
            await diesel.handle_makethread_lookup(
                "https://liquipedia.net/test", "template", 1, mock_channel
            )
    ...
    async def test_handle_broadcast_lookup(self):
        ...
        with patch(
            ...
            await diesel.handle_broadcast_lookup(
                "https://liquipedia.net/test", mock_channel
            )
            mock_print.assert_awaited_once()
    ...
    async def test_handle_stream_lookup(self):
        ...
        with patch(
            ...
            await diesel.handle_stream_lookup(
                "https://liquipedia.net/test", mock_channel
            )
            mock_print.assert_awaited_once()
    ...
    async def test_handle_schedule_lookup(self):
        ...
        with patch(
            ...
            await diesel.handle_schedule_lookup(
                "https://liquipedia.net/test", 1, mock_channel
            )
            mock_print.assert_awaited_once()
    ...
    async def test_handle_schedule_lookup_date(self):
        ...
        with patch(
            ...
            await diesel.handle_schedule_lookup_date(
                "https://liquipedia.net/test", 20220101, mock_channel
            )
            mock_print.assert_awaited_once()
    ...
    async def test_handle_coverage_lookup(self):
        ...
        with patch(
            ...
            await diesel.handle_coverage_lookup(
                "https://liquipedia.net/test", mock_channel
            )
            mock_print.assert_awaited_once()
